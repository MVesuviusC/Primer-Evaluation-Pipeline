...
colsForTarget <- list("On-target" = "#1b9e77",
                      "Off-target" = "#d95f02")
...
for(target in unique(primerMismatchLocs$OnTarget)) {
  ...
  print(primerMismatchLocs %>%
    filter(OnTarget == target) %>%
    ggplot(., aes(x = mismatchLoc, y = count/totalCount)) + 
      geom_bar(stat = "identity", fill = colsForTarget[[target]]) + 
      ...
      ylab("") + 
      scale_x_reverse(breaks = 1:max(primerMismatchLocs$mismatchLoc, na.rm = TRUE))
  )
  dev.off()
}
```
...
# Get rid of hits with only genus (no space in name)
distance <- distance[grep(" ", distance$species),]
...
Vasco Elbrecht brought up an important point. For sequences contributed to the database, the primers used to generate the sequence are usually trimmed. This means that any sequence in the database genrated using the primers currently tested will be defined as "unsequenced" because the sequence does not include the primer sequences.
...
ggplot(forCloud, aes(label = paste(Taxa, " (", Percent, "%)", sep = ""), size = log10Count, color = onTarget)) +
...
missedSummary <- potentialHits %>% 
  ...
  ungroup() %>%
  mutate(xlabs = paste(Taxa, "(", NumHit, ")", sep = ""))  
