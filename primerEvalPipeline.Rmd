...
# To do
  * I should move the primer file to ${outDir} so it is recorded
...
Sys.setenv(targetTaxa = params$targetTaxa)
```
...
colnames(summaryTable) <- c("PrimerFName", "PrimerF", "PrimerRName", "PrimerR", "TargetTaxa")
...
taxaSummaryOnTarget <- subset(taxaSummary, get(params$targetLevel) == params$targetTaxa)

summaryTable$onTargetSpeciesAmplifiedCount <- sum(taxaSummary[[params$targetLevel]] == params$targetTaxa, na.rm = T)
