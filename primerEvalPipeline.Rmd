...
distanceSummary <- distance %>%
  mutate(., OnTarget = grepl(targetTaxa, distance[[targetLevel]])) %>% 
...
for(levelInUse in levelsToUse) {
  ...
  tempDf <- distanceSummary %>% 
    select(., CompLevel, OnTarget, Average) %>% 
    distinct(.) %>% 
    filter(., OnTarget == TRUE, CompLevel == levelInUse)
  
  primerDf[[paste("MeanDist_", levelInUse, sep = "")]] <- tempDf$Average[1]
}
...
PercentFound <- (length(potentialSpecies) - length(missedSpecies)) / length(potentialSpecies) * 100
summaryTable$PercentTargetCovered <- PercentFound 
