...
Need to update bsPrimerTree to give --noPlots option
...
ampLenDf <- read.delim(paste(params$outDir, "/ampliconLengths.txt", sep = ""))
...
P2 <- ggplot(primerMismatches, aes(x = TipMismatches, y = Count)) + 
          geom_bar(stat = "identity") + 
          facet_wrap(~ Primer) +
          ggtitle("Mismatches in 5' end of primer") +
          xlab("") +
          ylab("Number of sequences")
...
### Trim primers off sequences
```{r, echo = F, cache = FALSE}
read_chunk('scripts/trimFastaByPrimerLength.pl', labels = 'trimPrimersCode')
```
```{perl trimPrimersCode, eval = FALSE, cache = FALSE}
```
```{bash trimPrimerSeqs, dependson = 'bsPrimerTree', eval = TRUE}
perl trimFastaByPrimerLength.pl \
    --fasta ${outDir}/seqsWithTaxa.fasta \
    --primers ${primerFile} \
      > ${outDir}/seqsWithTaxaTrimmed.fasta
```
...
############### remove need for taxa file and change up script
```{r, echo = F, cache = FALSE}
...
perl scripts/uniqueFastaBySpeciesSeq.pl \
    --fasta ${outDir}/seqsWithTaxaTrimmed.fasta \
      > ${outDir}/seqsWithTaxaTrimmedUnique.fasta
```
...
${blastLoc} \
    -task blastn \
    # maybe don't use this, need to think about it -negative_gilist misc/unculturedOrgs.gi \
    -db ${blastDb} \
    -query ${outDir}/seqsWithTaxaTrimmedUnique.fasta \
    -num_threads ${threads} \
    -outfmt "7 qseqid staxids evalue length qlen sstart send slen" \
    -max_hsps 1 \
...
```{bash getGi, dependson = 'blast', eval = FALSE}
...
local taxonomy database is created by makeTaxonomyDb.pl

```{bash getTaxonomy, dependson = 'getGi', eval = FALSE}


################################### do this part
perl getTaxaLocal.pl

cat output/blast/taxa/splitOutput/splitTaxa_* | grep -v "superkingdom" | grep -v "NotFound" >> ${outDir}/blastTaxa.txt
```
...
```
```{bash parseTopBlastHits, dependson = getTaxonomy, eval = FALSE}
perl scripts/findTopBlastHitsWithTaxa.pl \
    --blastIn ${outDir}/blastResults.txt.gz \
    --taxa ${outDir}/blastTaxa.txt \
