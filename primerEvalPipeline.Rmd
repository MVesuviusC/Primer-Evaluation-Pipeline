...
# To do
I should move the primer file to ${outDir} so it 
...
```{r bannedWords, eval = TRUE, cache = FALSE}
bannedWords <- c("sp.", "cf.", "isolate", "uncultured", "symbiont", "unidentified")
```

```{bash, writeBanned, eval = FALSE}
echo -e "sp.\ncf.\nisolate\nuncultured\nsymbiont\nunidentified" > ${outDir}/bannedWords.txt
```
...
```{bash bsPrimerTree, dependson = 'getPrimers', eval = FALSE}
bsPrimerBlast.pl \
...
```
```{bash uniqueSeqs, dependson = 'trimPrimerSeqs', eval = FALSE}
...
```{bash blast, dependson = 'uniqueSeqs', eval = FALSE}
${blastLoc} \
    -task blastn \
    -db ${blastDb} \
...
```{bash getTaxids, dependson = 'blast', eval = FALSE}
...
```{bash getTaxonomy, dependson = 'getTaxids', eval = FALSE}
...
perl scripts/countNumInRank.pl --input ${outDir}/blastParsed.txt --outDir ${outDir}
```
...
perl scripts/getPotentialHits.pl \
    --alignVar 10 \
    --taxa ${outDir}/taxaRaw.txt \
    --blast ${outDir}/blastResults.txt.gz \
    --primerFile ${primerFile} \
      > ${outDir}/potential_hits.txt
```
...
potentialHits <- read.delim(paste(params$outDir, "/potential_hits.txt", sep = ""))
# The taxa level should always be lowercase, but might be supplied uppercase
targetLevel <- tolower(params$targetLevel)
...
potentialHits <- subset(potentialHits, get(targetLevel) == params$targetTaxa)
...
print(paste(params$outDir, "/potentialSpeciesOnTarget.txt", sep = ""))

write.table(potentialSpecies, file = paste(params$outDir, "/potentialSpeciesOnTarget.txt", sep = ""))
```



```{bash listHitSpeciesDELETEME, dependson = 'getTaxonomy', eval = FALSE}
