...
bsPrimerTreeHits <- subset(bsPrimerTreeHits, get(targetLevel) == params$targetTaxa)
...
write.table(bsPrimerTreeHits, file = paste(params$outDir, "/bsPrimerTreeOut/SpeciesOnTarget.txt", sep = ""), sep = "\t", quote = F, row.names = F)
```
...
perl scripts/uniqueFastaBySpeciesSeq.pl \
    --fasta ${outDir}/bsPrimerTreeOut/seqsWithTaxa.fasta \
...
potentialHits <- read.delim(paste(params$outDir, "/potentialHits.txt", sep = ""))
# The taxa level should always be lowercase, but might be supplied uppercase
targetLevel <- tolower(params$targetLevel)
...
write.table(potentialSpecies, file = paste(params$outDir, "/potentialSpeciesOnTarget.txt", sep = ""), sep = "\t", quote = F, row.names = F)
```
